...
- **v1/v2/v3 보드 분리** — 이전 시즌 데이터를 아카이브(v1, v2)로 보존하고 v3("뉴 채용혁신개발팀")로 새로 시작

## 보드 버전

| 경로 | 설명 |
|------|------|
| `/board` | **v3 현재 보드** — 기본 진입점, 2026-06-23 이후 데이터, "뉴 채용혁신개발팀" |
| `/board/v2` | **2기 아카이브** — 읽기 전용, 2026-05-04 ~ 2026-06-23 데이터 |
| `/board/v1` | **1기 아카이브** — 읽기 전용, ~2026-05-04 데이터 |

- v3 헤더의 **"지난 대나무숲 다시보기"** 드랍다운으로 아카이브에 접근할 수 있습니다.
- 날짜 기반 분리(`src/config/boardVersion.ts`의 `V2_CUTOFF_DATE`, `V3_CUTOFF_DATE`)로 동작하므로 Supabase 마이그레이션 없이 동작합니다.
- 향후 v4 등 새 시즌을 시작할 때는 `ARCHIVE_BOARDS` 배열에 v3 항목을 추가하고, 새 `V4_CUTOFF_DATE`를 만들어 `BoardPage`의 버전별 날짜 필터에 반영하면 됩니다.
